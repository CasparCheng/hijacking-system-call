...
asmlinkage long my_exit_group(struct pt_regs reg)
{
	...
	/* exclusively operate on table to delete the exiting process's pid
	   under all registered syscalls */
	spin_lock(&my_table_lock);
	rc = del_pid(current->pid);
	spin_unlock(&my_table_lock);

	if (rc != 0) {
		/* the exiting process may not be monitored
		   do nothing here */
	}

	/* call original exit_group syscall */
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	mytable * tb;  /* pointer to entry in table */
	asmlinkage long (*orig_syscall)(struct pt_regs reg);  /* pointer to original syscall */
	int should_log;  /* log switch */

	syscall = reg.ax;
	should_log = 0;

	/* exclusively operate on table to find out if
	   current pid is being monitored for the given
	   syscall and determine if should log */
	spin_lock(&my_table_lock);
	tb = &table[syscall];
	orig_syscall = tb->f;
	if (tb->monitored == 0) {
		/* syscall is not monitored */
	} else if (tb->monitored == 1) {
		/* syscall is monitored for some pids */
		if (check_pid_monitored(syscall, current->pid) == 0) {
			/* pid is not being monitored for */
		} else {
			should_log = 1;
		}
	} else {
		/* syscall is monitored for all pids */
		should_log = 1;
	}
	spin_unlock(&my_table_lock);

	/* perform IO operation outside critical section */
	if (should_log) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	switch (cmd) {
		case REQUEST_SYSCALL_INTERCEPT:
			...
			spin_lock(&my_table_lock);
			tb = &table[syscall];
			/* check if syscall is already being intercepted */
			if (tb->intercepted) {
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
			tb->intercepted = 1;
			/* access sys_call_table */
			spin_lock(&sys_call_table_lock);
			tb->f = sys_call_table[syscall];
		...
		case REQUEST_SYSCALL_RELEASE:
			...
			spin_lock(&my_table_lock);
			tb = &table[syscall];
			/* check if syscall is not being intercepted */
			if (tb->intercepted == 0) {
				spin_unlock(&my_table_lock);
				return -EINVAL;
			}
			tb->intercepted = 0;
			/* delete all associated pids */
			destroy_list(syscall);
			/* access sys_call_table */
			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long)sys_call_table);
			/* restore original syscall */
			sys_call_table[syscall] = tb->f;
			set_addr_ro((unsigned long)sys_call_table);
			spin_unlock(&sys_call_table_lock);
			tb->f = NULL;
			spin_unlock(&my_table_lock);
		...
		case REQUEST_START_MONITORING:
			/* accept root or the process owner */
			...
			/* accept syscall that is already being intercepted */
			spin_lock(&my_table_lock);
			tb = &table[syscall];
			if (tb->intercepted == 0) {
				spin_unlock(&my_table_lock);
			...
			/* act on 'monitored' */
			if (tb->monitored == 0) {
				/* no pids are being monitored */
				if (pid == 0) {
					/* start monitoring for all pids */
					tb->monitored = 2;
				} else {
					/* start monitoring for pid */
				...
				/* some pids are being monitored for */
				if (pid == 0) {
					/* start monitoring for all pids */
					destroy_list(syscall);
					tb->monitored = 2;
				} else {
					/* check if pid is already being monitored for */
					if (check_pid_monitored(syscall, pid)) {
						/* pid is already being monitored for */
						spin_unlock(&my_table_lock);
						return -EBUSY;
					}
					/* start monitoring for pid */
				...
				/* all pids are being monitored for */
				/* blacklist mode */
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
		...
		case REQUEST_STOP_MONITORING:
			...
			/* act on 'monitored' */
			if (tb->monitored == 0) {
				/* no pids are being monitored for */
				spin_unlock(&my_table_lock);
				return -EINVAL;
			} else if (tb->monitored == 1) {
				/* some pids are being monitored for */
				if (pid == 0) {
					/* delete all pids under syscall */
					destroy_list(syscall);
					tb->monitored = 0;
				} else {
					/* try delete pid under syscall */
				...
				if (pid == 0) {
					...
					if (check_pid_monitored(syscall, pid)) {
						/* pid is already in blacklist under syscall */
						spin_unlock(&my_table_lock);
						return -EINVAL;
					}
					/* add pid in blacklist under syscall */
					if (add_pid_sysc(pid, syscall) != 0) {
						spin_unlock(&my_table_lock);
						return -ENOMEM;
					}
...
static int init_function(void) {
	...
	/* hijack MY_CUSTOM_SYSCALL and exit_group syscalls */
	/* save original syscalls */
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];

	/* access sys_call_table to replace syscalls */
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
...
{        
	...
	/* undo hijack MY_CUSTOM_SYSCALL and exit_group syscalls */
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	sys_call_table[__NR_exit_group] = orig_exit_group;

	/* undo hijack other syscalls and release memory allocated for lists */
	spin_lock(&my_table_lock);
	for (i = 0; i < NR_syscalls; ++i) {
		tb = &table[i];
		/* guarantee the original syscall is not NULL */
		if (tb->intercepted && tb->f) {
			sys_call_table[i] = tb->f;
		}
		destroy_list(i);
	}
	spin_unlock(&my_table_lock);
